...
async def list_qr_codes_endpoint(token: str = Depends(oauth2_scheme)):
    ...
    for qr_file in qr_files:
        if qr_file.endswith(".png"):
            qr_code_download_url = f"{SERVER_BASE_URL}/{SERVER_DOWNLOAD_FOLDER}/{qr_file}"
            responses.append(QRCodeResponse(
                message="QR code available",
                qr_code_url=decode_filename_to_url(qr_file[:-4]),
                links=generate_links("list", qr_file, SERVER_BASE_URL, qr_code_download_url)
            ))
    return responses
